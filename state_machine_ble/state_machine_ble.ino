...
String setupBLEList[NUM_SETUPOPERATIONS_BLE] = { "AT+IMME0", "AT+ROLE0", "AT+NAMEmeter1", "AT+NOTI1", "AT+NOTP1", "AT+ADTY0", "AT+BAUD0", "AT+ADVI1", "AT+SHOW3", "AT+PWRM1", "AT+RESET" };
...
const char *const stateName[] PROGMEM = { "INIT", "RESET", "SETUP_BLE", "BLE_ADV", "BLE_ADV_9", "BLE_ADV_D", "BLE_MASTER", "BLE_CONNECTED", "BLE_DISCONNECTION" };  // Helper for print labels instead integer when state change
...
void setupStateMachine() {
  ...
  stateMachine.AddState(stateName[BLE_CONNECTED], CONNECTION_TIME, onEnter, inTransmission, onExit);
  ...
  stateMachine.AddTransition(BLE_ADV_D, BLE_MASTER, bleMasterState);
